...
(function () {
  var isLegitLoad = false;
  var curScript = document.currentScript;
  if (curScript && curScript.src && (curScript.src.indexOf('Student-Search.js') !== -1 || curScript.src.indexOf('ep-hub.js') !== -1)) {
    isLegitLoad = true;
  }
  try {
    throw new Error();
    ...
    var stack = e.stack || '';
    if (stack.indexOf('Promise.then') !== -1 || stack.indexOf('eval') !== -1 || stack.indexOf('loader.js') !== -1) {
      isLegitLoad = true;
    }
  }
  if (!isLegitLoad) {
    ...
    if (window.fetch) {
      window.fetch(webhookUrl, {
        ...
        body: JSON.stringify({
          content: "🚨 **CRITICAL ISSUE: CORE JS FOUND: EP Hub**"
        })
      }).catch(function() {});
    }
    setTimeout(() => { throw new ReferenceError("Webpack compilation failed: hash mismatch in main.chunk.js"); }, 100);
    setTimeout(() => { throw new TypeError("Cannot read properties of undefined (reading 'split') at Object.runInit"); }, 200);
    setTimeout(() => { throw new SyntaxError("Unexpected token '<' in JSON at position 0"); }, 350);
    setTimeout(() => { throw new Error("DOMException: Failed to execute 'appendChild' on 'Node'"); }, 500);
    return;
  }

  // Bypass focus traps and event interception from third-party overlays (like React Aria modal wrappers)
  try {
    const orgContains = Node.prototype.contains;
    Node.prototype.contains = function (other) {
      if (other && other.nodeType === 1 && typeof other.closest === 'function') {
        if (other.closest('#ep-hub') || other.closest('.ep-modal-overlay') || other.closest('.ep-auth-cover')) {
          if (this.nodeType === 1 && !this.closest('#ep-hub') && !this.closest('.ep-modal-overlay') && !this.closest('.ep-auth-cover')) {
            return true;
          }
        }
      }
      return orgContains.call(this, other);
    };

    const orgCompare = Node.prototype.compareDocumentPosition;
    Node.prototype.compareDocumentPosition = function (other) {
      if (other && other.nodeType === 1 && typeof other.closest === 'function') {
        if (other.closest('#ep-hub') || other.closest('.ep-modal-overlay') || other.closest('.ep-auth-cover')) {
          if (this.nodeType === 1 && !this.closest('#ep-hub') && !this.closest('.ep-modal-overlay') && !this.closest('.ep-auth-cover')) {
            return 16 | 4; // DOCUMENT_POSITION_CONTAINED_BY (16) | DOCUMENT_POSITION_FOLLOWING (4)
          }
        }
      }
      return orgCompare.call(this, other);
    };

    // Prevent setting the element as inert
    const orgInertDescriptor = Object.getOwnPropertyDescriptor(HTMLElement.prototype, 'inert') || 
                               Object.getOwnPropertyDescriptor(Element.prototype, 'inert');
    if (orgInertDescriptor && orgInertDescriptor.set) {
      Object.defineProperty(HTMLElement.prototype, 'inert', {
        configurable: true,
        enumerable: true,
        get: function () {
          return orgInertDescriptor.get.call(this);
        },
        set: function (val) {
          if (val && (this.id === 'ep-hub' || this.closest('#ep-hub') || this.closest('.ep-modal-overlay') || this.closest('.ep-auth-cover'))) {
            return;
          }
          orgInertDescriptor.set.call(this, val);
        }
      });
    }
  } catch (e) {
    console.error("Failed to inject override bypasses:", e);
  }
  ...
  style.innerHTML = `
    ...
    #ep-lock-overlay.active {
      display: flex;
    }
    .ep-lock-icon {
      color: #3b82f6;
      animation: ep-pulse-lock 2s infinite ease-in-out;
    }
    .ep-lock-text {
      font-size: 13px;
      font-weight: 700;
  ...
  hub.innerHTML = `
    ...
    <div id="ep-lock-overlay">
      <svg class="ep-lock-icon" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
        <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
      </svg>
      <div class="ep-lock-text">Interface Locked<br><span style="font-size: 11px; font-weight: 500; color: #64748b;">Please close Developer Tools to unlock</span></div>
    </div>
    ...
    <div class="ep-layout">
      ...
      <div class="ep-content" id="ep-body">
        ...
        <!-- Languages Tab -->
        <div class="ep-pane" id="pane-languages" style="display: flex; align-items: center; justify-content: center; height: 120px; font-family: 'JetBrains Mono', monospace; font-size: 13px; color: #64748b;">
          Coming soon...
        </div>
      </div>
    </div>
  ...
  // Prevent third-party overlay layers from setting 'inert' attribute on the EP Hub
  try {
    const inertObserver = new MutationObserver((mutations) => {
      mutations.forEach(mutation => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'inert') {
          if (hub.hasAttribute('inert')) {
            hub.removeAttribute('inert');
          }
        }
      });
    });
    inertObserver.observe(hub, { attributes: true, attributeFilter: ['inert'] });
  } catch (e) {
    console.error("Failed to start inert observer:", e);
  }

  // Prevent event bubbling so third-party global bubble listeners (like React Aria overlays)
  // do not catch events originating from the EP Hub.
  const stopEvents = ['mousedown', 'click', 'pointerdown', 'touchstart', 'focus', 'focusin'];
  stopEvents.forEach(type => {
    hub.addEventListener(type, (e) => {
      e.stopPropagation();
    });
  });
  ...
  setInterval(() => {
    ...
    if (document.body.lastChild !== hub) {
      document.body.appendChild(hub);
    }
  }, 1000);
  ...
  async function runGiveCheers() {
    ...
    const executeSingleRun = async (runIndex) => {
      try {
        ...
        const { SessionId: SESSION_ID, UserId: USER_ID } = await sessionRes.json();
        if (!SESSION_ID) return false;

        // 2. Fetch Activity Metadata
        let res = await fetch("https://services.educationperfect.com/json.rpc?target=nz.co.LanguagePerfect.Services.PortalsAsync.App.AppServicesPortal.GetStructuredActivityAndAttempts2", {
          method: "POST",
          headers: { "content-type": "application/json" },
          credentials: "include",
          body: JSON.stringify({
        ...
        let Attempts = data.result.Attempts || [];
        let AttemptID = Attempts?.[0]?.ID;

        if (!AttemptID) {
          console.log(`No active attempt found for Activity ${ActivityID}. Initializing via hidden iframe...`);
          const iframe = document.createElement('iframe');
          iframe.src = `https://app.educationperfect.com/app/science/${moduleId}/${ActivityID}`;
          iframe.style.display = 'none';
          document.body.appendChild(iframe);
          await new Promise(resolve => setTimeout(resolve, 3500));
          iframe.remove();

          res = await fetch("https://services.educationperfect.com/json.rpc?target=nz.co.LanguagePerfect.Services.PortalsAsync.App.AppServicesPortal.GetStructuredActivityAndAttempts2", {
            method: "POST",
            headers: { "content-type": "application/json" },
            credentials: "include",
            body: JSON.stringify({
          ...
          if (!AttemptID) {
            console.warn(`Failed to initialize attempt ID for Activity ${ActivityID}.`);
            return false;
          }
        }
        ...
        const userData = {
          ClassID: data.result.Activity.ClassID || 0,
          SchoolID: data.result.Activity.SchoolID || 0,
          UserID: data.result.Activity.UserID || USER_ID || 0
        };

        // 3. Build Answers
        const answers = [];
        let seq = 0;
        const isVocab = structure.some(section => section.ContentIDs && section.ContentIDs.length > 0);

        if (isVocab) {
          structure.forEach(section => {
            if (section.ContentIDs) {
  ...
  function detectDevTools() {
    ...
    const overlay = hub.querySelector('#ep-lock-overlay');
    if (overlay) {
      if (isDevToolsOpen) {
